 ...
 *
 * Default mode opens the Equo documentation page:
 * ./gradlew run
 *
 * Pass -Pcsd to enable Client-Side Decorations mode, where the native OS
 * window decorations are replaced by HTML/CSS/JS elements rendered inside
 * the browser. The window.equo JavaScript API is injected automatically
 * and exposes:
 *
 *   window.equo.beginMove(screenX, screenY) - start a native window drag
 *   window.equo.beginResize(screenX, screenY, edge) - start a native resize
 *   window.equo.maximize() - maximize the window
 *   window.equo.restore() - restore from maximized state
 *   window.equo.minimize() - minimize the window
 *   window.equo.close() - close the window
 *
 * ./gradlew run -Pcsd
 */
class Standalone {}

private const val HTML = """<!doctype html><html><head><meta charset='utf-8'><style>
...
fun main() {
    if (java.lang.Boolean.getBoolean("csd")) {
        val url = "data:text/html;charset=utf-8," +
            URLEncoder.encode(HTML, StandardCharsets.UTF_8).replace("+", "%20")
        ChromiumBrowser.standalone(
            url,
